...
;;;-----------------------------------------------------------------------------
        ...
        VID              = 0x4711         ; Changed by LAP to match Altera VID
        PID              = 0x0023         ; Changed by LAP to match Altera USBBlaster PID 
        VERSION          = 0x0200         ; Product Version (4 indicates *BM device) 
        USB_VER          = 0x0200         ; Support USB version 1.10 
        USB_ATTR         = 0x80           ; Bus powered, not self-powered, no remote wakeup
        MAX_POWER        = 250             ; need 2*75 mA max
...
_dscr_attrpow::
        ...
        .db        DSCR_ENDPNT_LEN
        .db        DSCR_ENDPNT
        .db        (1 | USB_DIR_OUT) ; bEndpointAddress (ep 1 OUT)
        ...
        .db        DSCR_ENDPNT_LEN
        .db        DSCR_ENDPNT
        .db        (1 | USB_DIR_IN) ; bEndpointAddress (ep 1 IN)
        .db        ET_BULK          ; bmAttributes
        .db        <64              ; wMaxPacketSize (LSB)
        .db        >64              ; wMaxPacketSize (MSB)
        .db        0                ; bInterval (iso only)
        ...
        .db        DSCR_ENDPNT_LEN
        .db        DSCR_ENDPNT
        .db        (2 | USB_DIR_OUT) ; bEndpointAddress (ep 2 OUT)
        .db        ET_BULK          ; bmAttributes
        ...
        .db        DSCR_ENDPNT_LEN
        .db        DSCR_ENDPNT
        .db        (6 | USB_DIR_IN) ; bEndpointAddress (ep 6 IN)
        .db        ET_BULK          ; bmAttributes
        .db        <512              ; wMaxPacketSize (LSB)
        .db        >512              ; wMaxPacketSize (MSB)
        .db        0                ; bInterval (iso only)
